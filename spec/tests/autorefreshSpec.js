...
var SECRET = 'xxx';
var REQUIRED_CLAIMS = ['nbf', 'exp'];
var RECOMMENDED_CLAIMS = ['sub', 'aud'];

var generate = function generate() {
  var nbf = Date.now() / 1000;
  var claimsPass = [{ nbf: nbf, exp: nbf + 1 }, { nbf: nbf, exp: nbf + 10 }, { nbf: nbf, exp: nbf + 100 }];

  var claimsFail = [{ nbf: nbf, exp: nbf - 100 }, { nbf: Date.now() / 100, exp: Date.now() / 100 - 1 }, { nbf: Date.now() / 10, exp: Date.now() / 10 }, { nbf: Date.now() * -1, exp: Date.now() * -1 + 1 }];
  ...
  return { nbf: nbf, claimsPass: claimsPass, claimsFail: claimsFail, jwtPass: jwtPass, jwtFail: jwtFail, refreshPass: refreshPass, refreshFail: refreshFail, leadSecondsPass: leadSecondsPass, leadSecondsFail: leadSecondsFail };
};
